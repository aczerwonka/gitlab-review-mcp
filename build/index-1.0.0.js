...
class GitLabClient {
    ...
    async getMergeRequest(projectId, mrIid) {
        const mr = await this.request(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}`);
        return {
            title: mr.title,
    ...
    async getMergeRequestDiff(projectId, mrIid) {
        const diffs = await this.request(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/changes`).then((response) => response.changes || []);
        return {
    ...
    async postMergeRequestComment(projectId, mrIid, body) {
        const comment = await this.request(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/notes`, {
            method: "POST",
    ...
    async postMergeRequestInlineComment(projectId, mrIid, body, filePath, lineNumber, lineType = "new") {
        ...
        const comment = await this.request(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/discussions`, {
            method: "POST",
            body: JSON.stringify({
                body,
                position,
    ...
    async postMergeRequestInlineCommentsBatch(projectId, mrIid, comments) {
        ...
        for (const comment of comments) {
            try {
                ...
                await this.request(`/projects/${encodeURIComponent(projectId)}/merge_requests/${mrIid}/discussions`, {
                    method: "POST",
                    body: JSON.stringify({
                        body: comment.body,
                        position,
    ...
    async ensureReviewDir(workspaceRoot) {
    ...
    async saveDiffWithComments(workspaceRoot, projectId, mrIid) {
    ...
    async loadReviewFile(reviewFilePath) {
    ...
    async publishAcceptedComments(reviewFilePath, projectId, mrIid) {
        ...
        for (const comment of comments) {
            ...
            try {
                ...
                if (comment.line_number > 0 && comment.file_path) {
                    await this.postMergeRequestInlineComment(projectId, mrIid, comment.body, comment.file_path, comment.line_number, comment.line_type);
                    published++;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "post_merge_request_inline_comment") {
            const { project_id, mr_iid, body, file_path, line_number, line_type } = args;
            if (!project_id || typeof project_id !== "string") {
                throw new Error("project_id must be a non-empty string");
            }
            if (!mr_iid || typeof mr_iid !== "number") {
            ...
            const result = await gitlabClient.postMergeRequestInlineComment(project_id, mr_iid, body, file_path, line_number, line_type);
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "post_merge_request_inline_comments_batch") {
            const { project_id, mr_iid, comments } = args;
            if (!project_id || typeof project_id !== "string") {
                throw new Error("project_id must be a non-empty string");
            }
            if (!mr_iid || typeof mr_iid !== "number") {
            ...
            const result = await gitlabClient.postMergeRequestInlineCommentsBatch(project_id, mr_iid, comments);
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "save_review_file") {
            const { workspace_root, project_id, mr_iid } = args;
            ...
            const reviewFilePath = await gitlabClient.saveDiffWithComments(workspace_root, project_id, mr_iid);
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "publish_review_comments") {
            const { review_file_path, project_id, mr_iid } = args;
            ...
            const result = await gitlabClient.publishAcceptedComments(review_file_path, project_id, mr_iid);
            return {
                content: [
                    {
                        type: "text",
